"""
Replay OpenRouter + Ollama example executions.
...
from chronicon import replay, openrouter_llm_call, ollama_llm_call
from chronicon.log import ExecutionLog

# Import workflows directly
import importlib.util
spec = importlib.util.spec_from_file_location(
    "openrouter_ollama_example",
    os.path.join(os.path.dirname(__file__), "openrouter_ollama_example.py")
)
openrouter_ollama_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(openrouter_ollama_module)

code_analysis_pipeline = openrouter_ollama_module.code_analysis_pipeline
code_generation_with_review = openrouter_ollama_module.code_generation_with_review
documentation_and_code_together = openrouter_ollama_module.documentation_and_code_together
...
def main():
    ...
    if not openrouter_key:
        print("⚠️  OPENROUTER_API_KEY not set (not needed for replay, but good to have)")
    
    # Connect to the execution log database
    log = ExecutionLog(db_path="openrouter_ollama_example.db")
    
    # Get the most recent failed execution for each workflow
    print("=" * 70)
    print("Replaying OpenRouter + Ollama Executions")
    print("=" * 70)
    print()
    ...
    providers = {
        "openrouter": openrouter_llm_call(
